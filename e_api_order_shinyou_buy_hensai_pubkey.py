...
S_TATEBI_TYPE = '2'     # 21.建日種類（このサンプルコードでは、2:建日順、3:単価益順、4:単価損順 のみ指定可能。）
# --- 以上設定項目 -------------------------------------------------------------------------
...
S_SIZYOU_C = '00'       # 11.市場。  00:東証   現在(2021/07/01)、東証のみ可能。
S_CONDITION = '0'       # 13.執行条件。  0:指定なし、2:寄付、4:引け、6:不成。指し値は、0:指定なし。
S_ORDER_PRICE = '147'   # 14.注文値段。  *:指定なし、0:成行、上記以外は、注文値段。小数点については、関連資料:「立花証券・e支店・API、REQUEST I/F、マスタデータ利用方法」の「2-12. 呼値」参照。
S_ORDER_SURYOU = '100'  # 15.注文数量。
S_TATEBI_TYPE = '2'     # 21.建日種類（このサンプルコードでは、2:建日順、3:単価益順、4:単価損順 のみ指定可能。）
...
def func_set_sZyoutoekiKazeiC(
                                str_sGenkinShinyouKubun,    # 現金信用区分
...
def func_order_sinyou_buy_close(
                                int_p_no,
                                str_sIssueCode,         # 銘柄コード
                                str_sSizyouC,           # 市場（現在、東証'00'のみ）
                                str_sCondition,         # 執行条件
                                str_sOrderPrice,        # 価格
                                str_sOrderSuryou,       # 株数
                                str_sTatebiType,        # 建日種類
                                str_sSecondPassword,    # 第２パスワード
                                dic_login_property,     # ログインレスポンス
                                str_sJsonOfmt           # 表示形式
                            ):
    """ 
...
if __name__ == "__main__":
    ...
    if dic_login_property.get('sSinyouKouzaKubun') == '1' :
        ...
        dic_return = func_order_sinyou_buy_close(
                                                    my_p_no,
                                                    ...
                                                    S_TATEBI_TYPE,      # 21.建日種類（このサンプルコードでは、2:建日順、3:単価益順、4:単価損順 のみ指定可能。）
